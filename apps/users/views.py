...
class UserinfoView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if user_info_form.is_valid():
            ...
            return JsonResponse(user_info_form.errors,safe=False)
...
class UpdatePwdView(View):
    ...
    def post(self, request):
        ...
        if modify_form.is_valid():
            ...
            pwd2 = request.POST.get('password2', '')
            if pwd1 != pwd2:
                return JsonResponse({'status': 'fail', 'msg': '密码不一致'}, safe=False)
            user = request.user
            user.password = make_password(pwd1)
            user.save()
            return JsonResponse({'status': 'success', 'msg': '密码修改成功'}, safe=False)
        else:
...
class SendEmailView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if UserProfile.objects.filter(email=email):
            return JsonResponse({'email': '邮箱已经存在'}, safe=False)
...
class UpdateEmailView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if existed_records:
            ...
            return JsonResponse({'email': '验证码错误'}, safe=False)
